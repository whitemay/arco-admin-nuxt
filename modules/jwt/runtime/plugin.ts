...
import { addRouteMiddleware, defineNuxtPlugin, useAuth, useAuthState, useRuntimeConfig } from '#imports'
...
export default defineNuxtPlugin(async (nuxtApp) => {
  ...
  const { enableRefreshOnWindowFocus, enableRefreshPeriodically }
    = runtimeConfig.session as SessionConfig
  ...
  // Refetch interval
  let refetchIntervalTimer: NodeJS.Timeout
  ...
  nuxtApp.hook('app:mounted', () => {
    ...
    if (runtimeConfig.provider.type === 'refresh') {
      ...
      const { refresh, refreshToken } = useAuth() as any
