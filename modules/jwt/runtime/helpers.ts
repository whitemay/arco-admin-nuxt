 ...
 * @param type Backend type to be enforced (e.g.: `local` or `refresh`)
 */
export function useTypedBackendConfig<T extends SupportedAuthProviders>(runtimeConfig: ReturnType<typeof useRuntimeConfig>, _type: T): Extract<DeepRequired<AuthProviders>, { type: T }> {
  return runtimeConfig.public.auth.provider as unknown as Extract<
 ...
 * @param pointer
 */
export function jsonPointerGet(obj: Record<string, any>, pointer: string): string | Record<string, any> {
