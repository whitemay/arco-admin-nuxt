...
const defaultsByBackend: {
  [key in SupportedAuthProviders]: AuthProviders;
} = {
  local: {
    type: 'local',
    pages: {
...
export default defineNuxtModule<ModuleOptions>({
  ...
  setup(userOptions, nuxt) {
    ...
    const selectedProvider = userOptions.provider?.type ?? 'local'
    ...
    if (!isProduction) {
      logger.info(
        `Selected provider: ${selectedProvider}. Auth API location is ${options.computed.fullBaseUrl}`,
      )
    }
